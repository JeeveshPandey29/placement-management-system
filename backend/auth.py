    ...
    with get_db() as cur:
        try:
            ...
            return {"id": row[0], "email": row[1], "role": row[2],
                    "full_name": row[3], "must_change_password": bool(row[4])}
        except HTTPException:
            raise
        except Exception:
            # Fallback: must_change_password column may not exist yet
            cur.execute(
                "SELECT id, email, role, full_name FROM pms_users WHERE id = %s",
                (uid,)
            )
            row = cur.fetchone()
            if not row:
                raise HTTPException(status_code=401, detail="User not found")
            return {"id": row[0], "email": row[1], "role": row[2],
                    "full_name": row[3], "must_change_password": False}
